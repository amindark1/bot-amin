...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'قواینین گروه to:\n'..rules
end
local function modadd(msg)
  -- superuser and admins only (because sudo are always has privilege)
  if not is_admin(msg) then
    return "شما ادمین نیستی"
  end
  local data = load_data(_config.moderation.data)
  if data[tostring(msg.to.id)] then
...
local function demote(receiver, member_username, member_id)
  ...
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver, '@'..member_username..' از مدیریت برداشته شد.')
end
...
local function username_id(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if vusername == member then
      ...
      member_id = v.id
      if mod_cmd == 'ادمین شو' then
        return promote(receiver, member_username, member_id)
      elseif mod_cmd == 'ادمین نی' then
...
local function run(msg, matches)
  ...
  if matches[1] == 'اد گروه' then
    print("گروه "..msg.to.print_name.."("..msg.to.id..") ادد شد")
    return modadd(msg)
  end
   if matches[1] == 'حذف گروه' then
    print("گروه "..msg.to.print_name.."("..msg.to.id..") ریمو شد")
    return modrem(msg)
  end
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'chat_rename' then
      ...
      elseif group_name_lock == 'غیر فعال' then
        return nil
      end
    end
    if matches[1] == 'تغیر اسم' and is_momod(msg) then
    ...
    if matches[1] == 'تغیر عکس' and is_momod(msg) then
      data[tostring(msg.to.id)]['settings']['set_photo'] = 'waiting'
      save_data(_config.moderation.data, data)
      return 'Please send me new group photo now'
    end

    if matches[1] == 'ادمین شو' and matches[2] then
      if not is_owner(msg) then
        return "Only owner can promote"
      end
      local member = string.gsub(matches[2], "@", "")
      local mod_cmd = 'ادمین شو' 
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted @".. member)
      chat_info(receiver, username_id, {mod_cmd= mod_cmd, receiver=receiver, member=member})
    end
    if matches[1] == 'ادمین نی' and matches[2] then
      if not is_owner(msg) then
    ...
    if matches[1] == 'اماراین' and is_momod(msg) then 
      local cbres_extra = {
        chatid = msg.to.id
      }
...
return {
  ...
  "(ادمین)$",
  "(اماراین) (.*)$",
  "(ست ادمین) (%d+) (%d+)$",-- (group id) (owner id)
  "(نیست) (.*)$",
  "(ضداسپم) (%d+)$",
  "(تنظیمات)$",
  "(مدیر ها)$",
